...
my_list = []
with open(Path(Path.cwd(), "attachment_links.csv"), 'r') as my_file:
...
class PdfNamesSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		for url in pdf_urls:
			...
			self.driver.get(url)
			WebDriverWait(self.driver, timeout).until(EC.visibility_of_element_located((By.XPATH, "//span[@class='vieweractionsbar-filename']") ))			
