...
class DomyPoznanSpider(scrapy.Spider):  # update class to DomyPoznan 
	name = 'domy_warszawa_test'          		#  if you use base of spider but want to scrape other page, you need to updatenem of the spider 
	...
	def parse(self, response):
		...
		self.driver.get('https://www.otodom.pl/sprzedaz/warszawa/?search%5Bdescription%5D=1&search%5Bregion_id%5D=7&search%5Bsubregion_id%5D=197&search%5Bcity_id%5D=26&nrAdsPerPage=72')
		sleep(3)
		...
		# = sel.xpath('//ul[contains(@class, "pager")]/li/a/text()')[-1].extract()

		""" 
		USE NUMBER OF PAGES TO SCRAPE TO CREATE A LIST OF URLS FOR THE ITERATION
		"""
		urls = []	
		for item in range(160,162):
			urls.append(f"https://www.otodom.pl/sprzedaz/warszawa/?search%5Bdescription%5D=1&search%5Bregion_id%5D=7&search%5Bsubregion_id%5D=197&search%5Bcity_id%5D=26&nrAdsPerPage=72&page={item + 1}")
